...
import { getAlHabeedSocket } from "./socket";
import { readAlHabeedSession, saveAlHabeedSession } from "./session";
...
const socket = getAlHabeedSocket();
...
export interface AlHabeedController {
  hub: ReactNode;
  local: ReactNode;
  create: ReactNode;
  join: ReactNode;
  room: (code: string) => { node: ReactNode; active: boolean };
  profilePanel: ReactNode;
  onProfile: () => void;
}

export function AlHabeedOnline({ children }: { children: (controller: AlHabeedController) => ReactNode }) {
  const routerLocation = useLocation();
  const navigate = useNavigate();
  const legacyRoomRef = useRef(new URLSearchParams(window.location.search).get("room") ?? "");
  const [room, setRoom] = useState<Room | null>(null);
  const [session, setSession] = useState<Session | null>(() => readAlHabeedSession());
  ...
  const setScreen = (next:"home"|"create"|"join"|"local") => {
    const routes = { home:"/", create:"/games/alhabeed/create", join:"/games/alhabeed/join", local:"/games/alhabeed/local" };
  ...
  const roomView = room && session && me ? <main className="app-shell">
  ...
  const createView = <Home screen="create" setScreen={setScreen} name={name} setName={setName} code={code} setCode={setCode} create={create} join={join} settings={settings} setSettings={setSettings} availableCategories={availableCategories} error={error} profile={profile} openProfile={()=>setProfileOpen(true)} />;
  const joinView = <Home screen="join" setScreen={setScreen} name={name} setName={setName} code={code} setCode={setCode} create={create} join={join} settings={settings} setSettings={setSettings} availableCategories={availableCategories} error={error} profile={profile} openProfile={()=>setProfileOpen(true)} />;
  return children({
    hub: home,
    local: <AlHabeedLocal onExit={()=>setScreen("home")} />,
    create: createView,
    join: joinView,
    room: (routeCode) => room && session && me && room.code === routeCode
      ? { node: roomView, active: true }
      : { node: <Home screen="join" setScreen={setScreen} name={name} setName={setName} code={code || routeCode} setCode={setCode} create={create} join={join} settings={settings} setSettings={setSettings} availableCategories={availableCategories} error={error} profile={profile} openProfile={()=>setProfileOpen(true)} />, active: false },
    profilePanel,
...
function saveAndEnter(result:any,name:string,setSession:any,setRoom:any,navigate:(to:string)=>void){const s={code:result.room.code,playerId:result.player.id,name,token:result.reconnectToken};saveAlHabeedSession(s);setSession(s);setRoom(result.room);navigate(`/room/${s.code}`)}
